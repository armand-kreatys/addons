...
class ParcMachine(models.Model):
    ...
    # description = fields.Json(string='Description de l\'article',
    ...
    def send_email_to_mariette(self):
        template = self.env.ref('parc_machine.email_template_hors_service')
        if template:
            template.send_mail(self.id, force_send=True)
            _logger.info(f"Email sent to Mariette for record {self.id}")
        else:
            _logger.error(
                "Email template 'parc_machine.email_template_hors_service' not found")
